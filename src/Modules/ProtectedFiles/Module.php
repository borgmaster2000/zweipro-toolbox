...
class Module implements ModuleInterface
{
	...
	const GUARD_FILE  = 'zweipro-guard.php';
    ...
    private function ensure_storage_dir(): void
{
    ...
    // Direkten Zugriff immer über Guard routen (Login + Rollencheck)
    $this->ensure_guard_runtime();
...
/* ====================== PERSISTENT GUARD (FAIL-SAFE) ====================== */

private function guard_path(): string
{
    return $this->get_storage_path() . '/' . self::GUARD_FILE;
}

private function htaccess_path(): string
{
    return $this->get_storage_path() . '/.htaccess';
}

private function write_htaccess_rewrite(): void
{
    $content = "<IfModule mod_rewrite.c>\n"
             . "RewriteEngine On\n"
             . "RewriteRule ^" . self::GUARD_FILE . "$ - [L]\n"
             . "RewriteRule ^(.*)$ " . self::GUARD_FILE . "?path=$1 [L,QSA]\n"
             . "</IfModule>\n";

    @file_put_contents($this->htaccess_path(), $content);
}

private function write_guard_php(): void
{
    $settings = get_option(self::OPTION_KEY, []);
    $redirect_id  = (int) ($settings['redirect_page_id'] ?? 0);
    $redirect_url = $redirect_id ? get_permalink($redirect_id) : home_url('/');

    $guard = "<?php\n"
        . "// Auto-generated by Zweipro ProtectedFiles - do not edit.\n"
        . "header('X-Robots-Tag: noindex, nofollow', true);\n"
        . "header('Cache-Control: private, no-store, no-cache, must-revalidate, max-age=0', true);\n\n"
        . "function zweipro_find_wp_load(\$start, \$max = 12) {\n"
        . "  \$dir = \$start;\n"
        . "  for (\$i=0; \$i<\$max; \$i++) {\n"
        ...
        . "  return null;\n"
        . "}\n\n"
        . "\$wp_load = zweipro_find_wp_load(dirname(__FILE__));\n"
        . "if (!\$wp_load) { http_response_code(500); echo 'wp-load.php not found'; exit; }\n"
        . "require_once \$wp_load;\n\n"
        . "\$opt = get_option(" . var_export(self::OPTION_KEY, true) . ", []);\n"
        . "\$files = \$opt['files'] ?? [];\n"
        . "\$redirect = " . var_export($redirect_url, true) . ";\n\n"
        . "\$rel = isset(\$_GET['path']) ? (string)\$_GET['path'] : '';\n"
        . "\$rel = ltrim(\$rel, '/');\n"
        . "\$rel = str_replace([\"\\0\", \"..\"], '', \$rel);\n"
        . "if (\$rel === '' || substr(\$rel, -1) === '/') { http_response_code(404); exit; }\n\n"
        . "\$up = wp_upload_dir();\n"
        . "\$base_dir = rtrim(\$up['basedir'], '/\\\\') . '/" . self::STORAGE_DIR . "/';\n"
        . "\$full = \$base_dir . \$rel;\n"
        . "\$rp = realpath(\$full);\n"
        . "\$rb = realpath(\$base_dir);\n"
        . "if (!\$rp || !\$rb || strpos(\$rp, \$rb) !== 0 || !is_file(\$rp)) { http_response_code(404); exit; }\n\n"
        . "\$bn = basename(\$rel);\n"
        . "\$token = '';\n"
        . "if (preg_match('~^([a-f0-9]{32})-~i', \$bn, \$m)) { \$token = strtolower(\$m[1]); }\n"
        . "elseif (preg_match('~^([a-f0-9]{32})\\.png$~i', \$bn, \$m)) { \$token = strtolower(\$m[1]); }\n"
        . "else { if (\$redirect) { header('Location: '.\$redirect, true, 302); exit; } http_response_code(403); echo 'Forbidden'; exit; }\n\n"
        . "\$exclude = [];\n"
        . "foreach (\$files as \$f) {\n"
        . "  if (!empty(\$f['token']) && strtolower((string)\$f['token']) === \$token) {\n"
        . "    if (isset(\$f['exclude_roles']) && is_array(\$f['exclude_roles'])) { \$exclude = \$f['exclude_roles']; }\n"
        . "    break;\n"
        . "  }\n"
        . "}\n\n"
        . "if (!is_user_logged_in()) { if (\$redirect) { header('Location: '.\$redirect, true, 302); exit; } http_response_code(403); echo 'Forbidden'; exit; }\n"
        . "\$u = wp_get_current_user();\n"
        . "\$roles = (array)\$u->roles;\n"
        . "if (!empty(array_intersect(\$roles, (array)\$exclude))) { if (\$redirect) { header('Location: '.\$redirect, true, 302); exit; } http_response_code(403); echo 'Forbidden'; exit; }\n\n"
        . "\$mime = function_exists('mime_content_type') ? mime_content_type(\$rp) : 'application/octet-stream';\n"
    ...
    @file_put_contents($this->guard_path(), $guard);
}

private function ensure_guard_runtime(): void
{
    $this->write_guard_php();
    $this->write_htaccess_rewrite();
}

private function remove_guard_files(): void
{
    $g = $this->guard_path();
    $h = $this->htaccess_path();
    if (file_exists($g)) { @unlink($g); }
    if (file_exists($h)) { @unlink($h); }
}
    ...
    public function render_admin_page(): void
{
    $settings = get_option(self::OPTION_KEY, []);
$path = $this->get_storage_path();
    if (!isset($settings['persist_protection_on_deactivate'])) {
        $settings['persist_protection_on_deactivate'] = 1;
    }
...
if (file_exists($thumb)) {
    ...
    // Redirect-Seite speichern
    $settings['redirect_page_id'] = isset($_POST['redirect_page_id'])
        ? intval($_POST['redirect_page_id'])
        : 0;

    // NEU: Schutz bei Deaktivierung behalten (Default: EIN)
    $settings['persist_protection_on_deactivate'] =
        !empty($_POST['persist_protection_on_deactivate']) ? 1 : 0;

    update_option(self::OPTION_KEY, $settings);

    // NEU: Guard + .htaccess sofort aktualisieren
    $this->ensure_storage_dir();
...
<script>
document.addEventListener('DOMContentLoaded', function () {
    const cb = document.getElementById('zweipro-persist-guard');
    if (!cb) return;

    cb.addEventListener('change', function () {
        if (!cb.checked) {
            const ok = confirm(
                "Bist du sicher, dass du den Schutz bei Deaktivierung aufheben willst?\n\n" +
                "Wenn das Plugin deaktiviert wird, sind die Dateien dann direkt erreichbar."
            );
